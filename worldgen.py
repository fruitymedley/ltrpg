# -*- coding: utf-8 -*-
"""
Created on Tue Aug  1 11:33:29 2023
...
random.seed(4097)
four = np.sqrt(random.uniform(0, 1, (size, size))) * np.exp(1.j * np.random.uniform(0, 2 * np.pi, (size, size)))
...
def filt(x, y):
    ...
    l4 = 1e-1 # high bump
    l5 = 4e-1 # low pass
    return g * (1.j * r / l0) / (1 + 1.j * r / l0) / (1 + r * 1.j / l1) / (1 + r * 1.j / l2) \
        * (1.j * r / l3) / (1 + r * 1.j / l3) * (1 + 1.j * r / l4) / (1 + r * 1.j / l5)
...
lpfs = [np.fft.ifft(1 / np.power(1 + 1.j * ws / np.tan(k), 4)) for k in np.linspace(np.pi - 0.5 * np.pi / 1000, 0.5 * np.pi / 1000, 1000, True)]
elevation = np.array([np.convolve(np.concatenate([e, e[:-1]]), lpf, 'valid') for e, lpf in zip(elevation, lpfs)])
