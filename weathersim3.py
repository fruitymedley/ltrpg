...
class World:
    ...
    def __init__(self, elevation: np.ndarray, scale: float=1, depth: float=12, tilt: float=23, 
                 lenYear: int=365, lenDay: int=24, day: int=0) -> None:
        ...
        self.Depth = depth
        self.Tilt = tilt
        self.LongitudesNodes = np.linspace(np.pi/self.SizeX,2*np.pi-np.pi/self.SizeX,self.SizeX,True)
        self.LatitudesNodes = np.arcsin(np.linspace(-1+1/self.SizeY,1-1/self.SizeY,self.SizeY,True))
        self.LongitudesFaces = np.linspace(0,2*np.pi*(1-1/self.SizeX),self.SizeX,True)
        ...
        self.Xs, self.Ys, self.Zs = np.meshgrid(
            np.arange(self.SizeX), 
            np.arange(self.SizeY), 
            np.arange(self.SizeZ)
        )
    ...
    def Update(self, intensity) -> None:
        ...
        # Earth
        temperature[:,:,0] += \
            World.earthK * self.AreaX[:,:,0] * dt * (np.roll(self.Temperature, -1, axis=0)[:,:,0] - self.Temperature[:,:,0]) / (self.Radius * np.cos(self.LatitudesNodes) * (self.LongitudesNodes - np.roll(self.LongitudesNodes, -1, axis=0))[:,:,0] * World.earthCp * self.Mass[:,:,0]) \
            + World.earthK * np.roll(self.AreaX[:,:,0], 1, axis=0) * dt * (np.roll(self.Temperature, 1, axis=0)[:,:,0] - self.Temperature[:,:,0]) / (self.Radius * np.cos(self.LatitudesNodes) * (np.roll(self.LongitudesNodes, 1, axis=0) - self.LongitudesNodes)[:,:,0] * World.earthCp * self.Mass[:,:,0]) \
            + World.earthK * self.AreaY[:,:-1,0] * dt * (np.roll(self.Temperature, -1, axis=1)[:,:,0] - self.Temperature[:,:,0]) / (self.Radius * (np.roll(self.LatitudesNodes, 1, axis=0) - self.LatitudesNodes)[:,:,0] * World.earthCp * self.Mass[:,:,0]) \
            + World.earthK * self.AreaY[:,1:,0] * dt * (np.roll(self.Temperature, 1, axis=1)[:,:,0] - self.Temperature[:,:,0]) / (self.Radius * (np.roll(self.LatitudesNodes, 1, axis=0) - self.LatitudesNodes)[:,:,0] * World.earthCp * self.Mass[:,:,0]) \
            + (self.Volume[:,:,1] > 0).astype(int) * ((np.roll(self.ElevationsNodes, 1, axis=2) - self.ElevationsNodes)[:,:,0] * World.earthK + (np.roll(self.ElevationsNodes, 2, axis=2) - np.roll(self.ElevationsNodes, 1, axis=2))[:,:,0] * World.waterK) / (np.roll(self.ElevationsNodes, 2, axis=2) - self.ElevationsNodes) * self.AreaZ[:,:,1] * dt * (np.roll(self.Temperature, 1, axis=2)[:,:,0] - self.Temperature[:,:,0]) / (self.Radius * (np.roll(self.ElevationsNodes, 1, axis=2) - self.ElevationsNodes)[:,:,0] * World.earthCp * self.Mass[:,:,0]) \
            + (self.Volume[:,:,1] == 0).astype(int) * ((np.roll(self.ElevationsNodes, 1, axis=2) - self.ElevationsNodes)[:,:,0] * World.earthK + (np.roll(self.ElevationsNodes, 3, axis=2) - np.roll(self.ElevationsNodes, 2, axis=2))[:,:,0] * World.airK) / (np.roll(self.ElevationsNodes, 3, axis=2) - self.ElevationsNodes) * self.AreaZ[:,:,2] * dt * (np.roll(self.Temperature, 2, axis=2)[:,:,0] - self.Temperature[:,:,0]) / (self.Radius * (np.roll(self.ElevationsNodes, 2, axis=2) - self.ElevationsNodes)[:,:,0] * World.earthCp * self.Mass[:,:,0]) \
            + (self.Volume[:,:,1] == 0).astype(int) * self.AreaZ[:,:,1] * dt / (self.Mass * self.earthCp) * intensity \
            - (self.Volume[:,:,1] == 0).astype(int) * self.AreaZ[:,:,1] * dt * World.sigma * np.power(self.Temperature[:,:,0], 4) / (self.Mass[:,:,0] * self.earthCp)
            
        # Water
        with np.errstate(divide='ignore', invalid='ignore'):
            temperature[:,:,1] += np.nan_to_num(\
                World.waterK * self.AreaX[:,:,1] * dt * (np.roll(self.Temperature, -1, axis=0)[:,:,1] - self.Temperature[:,:,1]) / (self.Radius * np.cos(self.LatitudesNodes) * (self.LongitudesNodes - np.roll(self.LongitudesNodes, -1, axis=0))[:,:,1] * World.waterCp * self.Mass[:,:,1]) \
                + World.waterK * np.roll(self.AreaX[:,:,1], 1, axis=0) * dt * (np.roll(self.Temperature, 1, axis=0)[:,:,1] - self.Temperature[:,:,1]) / (self.Radius * np.cos(self.LatitudesNodes) * (np.roll(self.LongitudesNodes, 1, axis=0) - self.LongitudesNodes)[:,:,1] * World.waterCp * self.Mass[:,:,1]) \
                + World.waterK * self.AreaY[:,:-1,1] * dt * (np.roll(self.Temperature, -1, axis=1)[:,:,1] - self.Temperature[:,:,1]) / (self.Radius * (np.roll(self.LatitudesNodes, 1, axis=0) - self.LatitudesNodes)[:,:,1] * World.waterCp * self.Mass[:,:,1]) \
                + World.waterK * self.AreaY[:,1:,1] * dt * (np.roll(self.Temperature, 1, axis=1)[:,:,1] - self.Temperature[:,:,1]) / (self.Radius * (np.roll(self.LatitudesNodes, 1, axis=0) - self.LatitudesNodes)[:,:,1] * World.waterCp * self.Mass[:,:,1]) \
                + ((np.roll(self.ElevationsNodes, 1, axis=2) - self.ElevationsNodes)[:,:,0] * World.earthK + (np.roll(self.ElevationsNodes, 2, axis=2) - np.roll(self.ElevationsNodes, 1, axis=2))[:,:,0] * World.waterK) / (np.roll(self.ElevationsNodes, 2, axis=2) - self.ElevationsNodes) * self.AreaZ[:,:,1] * dt * (np.roll(self.Temperature, 1, axis=2)[:,:,0] - self.Temperature[:,:,0]) / (self.Radius * (np.roll(self.ElevationsNodes, 1, axis=2) - self.ElevationsNodes)[:,:,0] * World.waterCp * self.Mass[:,:,0]) \
                + ((np.roll(self.ElevationsNodes, 2, axis=2) - self.ElevationsNodes)[:,:,0] * World.waterK + (np.roll(self.ElevationsNodes, 3, axis=2) - np.roll(self.ElevationsNodes, 2, axis=2))[:,:,0] * World.airK) / (np.roll(self.ElevationsNodes, 3, axis=2) - self.ElevationsNodes) * self.AreaZ[:,:,2] * dt * (np.roll(self.Temperature, 2, axis=2)[:,:,0] - self.Temperature[:,:,0]) / (self.Radius * (np.roll(self.ElevationsNodes, 2, axis=2) - self.ElevationsNodes)[:,:,0] * World.waterCp * self.Mass[:,:,0]) \
                + self.AreaZ[:,:,2] * dt / (self.Mass * self.earthCp) * intensity \
                - self.AreaZ[:,:,2] * dt * World.sigma * np.power(self.Temperature[:,:,1], 4) / (self.Mass[:,:,1] * self.waterCp)
            )
            # v_x = dP/dx / p
            ...
            velocityY[:,:,1] += (
                ...
                (
                    ...
                    np.square((
                        np.pad(np.roll(self.VelocityX[:,:,1], 0, axis=0), ((0,0),(0,1),(0,0))) + 
                        np.pad(np.roll(self.VelocityX[:,:,1], 0, axis=0), ((0,0),(0,1),(0,0)))
                    ) / 2) - 
                    # v_2^2
                    np.square((
                    ...
                    np.square((
                        np.pad(np.roll(self.VelocityX[:,:,1], -1, axis=0), ((0,0),(0,1),(0,0))) + 
                        np.pad(np.roll(self.VelocityX[:,:,1], -1, axis=0), ((0,0),(0,1),(0,0)))
                    ) / 2)
                )
                / (
